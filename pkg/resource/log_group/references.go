...
func (rm *resourceManager) ClearResolvedReferences(res acktypes.AWSResource) acktypes.AWSResource {
	...
	for f0idx, f0iter := range ko.Spec.SubscriptionFilters {
		if f0iter.RoleRef != nil {
			ko.Spec.SubscriptionFilters[f0idx].RoleARN = nil
		}
	}
...
func (rm *resourceManager) ResolveReferences(
	...
	if fieldHasReferences, err := rm.resolveReferenceForSubscriptionFilters_RoleARN(ctx, apiReader, ko); err != nil {
		return &resource{ko}, (resourceHasReferences || fieldHasReferences), err
	} else {
		resourceHasReferences = resourceHasReferences || fieldHasReferences
	}
...
// identifier field.
func validateReferenceFields(ko *svcapitypes.LogGroup) error {

	if ko.Spec.KMSKeyRef != nil && ko.Spec.KMSKeyID != nil {
		return ackerr.ResourceReferenceAndIDNotSupportedFor("KMSKeyID", "KMSKeyRef")
	}

	for _, f0iter := range ko.Spec.SubscriptionFilters {
		if f0iter.RoleRef != nil && f0iter.RoleARN != nil {
			return ackerr.ResourceReferenceAndIDNotSupportedFor("SubscriptionFilters.RoleARN", "SubscriptionFilters.RoleRef")
		}
	}
	return nil
}
...
// resolveReferenceForSubscriptionFilters_RoleARN reads the resource referenced
// from SubscriptionFilters.RoleRef field and sets the SubscriptionFilters.RoleARN
// from referenced resource. Returns a boolean indicating whether a reference
// contains references, or an error
func (rm *resourceManager) resolveReferenceForSubscriptionFilters_RoleARN(
	ctx context.Context,
	apiReader client.Reader,
	ko *svcapitypes.LogGroup,
) (hasReferences bool, err error) {
	for f0idx, f0iter := range ko.Spec.SubscriptionFilters {
		if f0iter.RoleRef != nil && f0iter.RoleRef.From != nil {
			...
			if arr.Name == nil || *arr.Name == "" {
				return hasReferences, fmt.Errorf("provided resource reference is nil or empty: SubscriptionFilters.RoleRef")
			}
			namespace, err := ackrt.ResolveCrossNamespaceReference(
				ctx,
				rm.cfg.EnableCrossNamespace,
			...
			ko.Spec.SubscriptionFilters[f0idx].RoleARN = (*string)(obj.Status.ACKResourceMetadata.ARN)
		}
	}
